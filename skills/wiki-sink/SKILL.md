...
4. For stable knowledge writes, read the relevant current canonical index before choosing the owner page:
   ...
   - `05-models/README.md` for the model catalog: which model family pages exist, what each page carries, and where a reader should start.
   - `05-models/<topic>.md` for model families, member system-understanding models, model relationships, key fields, demo/examples, and source-of-truth facts.
...
5. Read the matching writing guidance before editing. When writing or updating `wiki/01-system.md`, read `../references/writing-guidance/system-overview.md`.
   ...
   - When writing or updating any non-README model family page under `wiki/05-models/**`, read `../references/writing-guidance/model-page.md`.
   - When writing or updating `wiki/05-models/README.md`, treat it as the Model Catalog README, not a model family detail page. Its main job is to introduce the other pages in the directory: page, carried content, when to read it, and related flow / module / decision / evidence. Keep any canonical model index compact and page-routing oriented; do not duplicate detail-page relationships, member definitions, key fields, lifecycle, demo/example, source-of-truth facts, or long writing guidance in README.
   - When choosing or creating a `wiki/05-models/<topic>.md` owner page, group models by shared reader question, stable fact chain, lifecycle, source-of-truth relationship, or demo/example explainability. In the page's `定位与边界`, first state the core reader question, then prefer a Mermaid usage context diagram that shows how the member models are created, read, referenced, derived, displayed, validated, expired, or terminated in important activity scenarios, execution chains, or code paths. Use `flowchart` when branches, abnormal paths, or result variants matter; use `sequenceDiagram` when cross-client, cross-module, cross-system, async, callback, or timing order matters; use multiple titled diagrams only for distinct core usage paths. Every usage context diagram must have an explicit entry and observable exit. A `flowchart` may include fallback or retry edges only when each back edge labels the business condition, user action, or system action; do not leave unlabeled closed loops. If a loop really expresses message return, callback, polling, or actor order, use `sequenceDiagram`; if it expresses stable state changes, use state-transition/lifecycle; if it expresses model references or derivation, move it to `模型关系`. Keep models in edge labels, messages, or notes, not as actors or activity nodes. For `sequenceDiagram`, participant headers must follow `../references/writing-blocks/sequence.md`: keep system participants at one abstraction level, and do not use result cards, state changes, models, payloads, or fact conclusions as headers unless they are confirmed active participants that send or receive messages. Simple or under-evidenced cases may use short prose or a small reader-entry table instead of a diagram. After the usage context entry, use at most one short range sentence for the main member models. Do not add separate `包含` lists, large exclusion lists, or terminology paragraphs after the entry; route those facts to `模型关系`, `成员模型`, or `相关页面` when they are still needed. In `成员模型`, each member model should start with a prominent definition and a compact `jsonc` structured example that follows `../references/writing-blocks/structured-example.md`; boundary, field meaning, and evidence should be short supporting notes instead of the main shape. Before finishing a model page, cross-check every Model node in `模型关系` against `成员模型`: main models need normal member subsections, and adjacent graph nodes need relationship-anchor subsections with minimal supported fields and continue-reading routes. Do not leave relation graph nodes unexplained just because they are not this page's full owner. Do not group or split model pages by code directory, table name, DTO/package name, field similarity, or because each model seems individually important.
...
6. Read the relevant writing blocks for the content shape:
   ...
   - `model-relation.md` for model relationship graph body and relation evidence; `structured-example.md` for member model mock data; `state-transition.md` for stable states. Use `model-page.md` guidance for lifecycle and source-of-truth facts.
