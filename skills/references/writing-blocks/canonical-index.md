...
```md
| Canonical name | Meaning / boundary | Owner page | Related pages | Evidence |
| --- | --- | --- | --- | --- |
| ... | ... | ... | ... | ... |
```
...
- `wiki-doctor` 做 reader-facing refresh 时，可以从已有 wiki 内容重建、补齐或调整 canonical index，但只能处理不改变含义的 `safe_guidance_rewrite`。它不能通过代码对照发明新名称，不能决定哪个冲突名称正确。
