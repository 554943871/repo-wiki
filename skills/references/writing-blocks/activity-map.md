...
优先使用 Mermaid `flowchart` 或活动表。信息较少时用表格更清楚；分支和汇合较多时用 Mermaid 更清楚。

```md
| Step | Actor | Condition | Business activity | Branch / join | Evidence |
| --- | --- | --- | --- | --- | --- |
| 1 | Buyer | Cart is ready | Submit checkout request | Mainline starts | `src/checkout/routes.ts` |
| 2 | Payment Provider | Payment approved | Confirm payment result | Joins fulfillment path | `/webhooks/payment` |
| 3 | Support Agent | Payment failed or risk flagged | Review failed order | Abnormal exit or manual recovery | `docs/support-orders.md` |
```
