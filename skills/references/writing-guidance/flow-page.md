...
- 简短流程定位。
- 参与者和触发条件。
- Activity map，用 Mermaid 主活动图表达业务主链路、actors、conditions、branches、joins、exceptions 和 cross-role handoffs；很短的线性 flow 才用步骤表作为主表达。
- Sequence diagram，用来解释关键场景中的参与方协作和调用顺序；很短的线性序列才用步骤表作为主表达。
...
| 表达 | 适合回答 | 何时使用 | 不适合 |
| --- | --- | --- | --- |
| Activity map | 谁在什么条件下做什么，主链路如何分支、汇合、异常退出和跨角色交接 | flow 有多个 actor、业务条件、异常路径、人工介入或跨系统/跨角色交接时；复杂 flow 默认用 Mermaid `flowchart` | 普通 Controller -> Service -> Repository 调用链，或只能列出文件和 payload 的实现说明 |
...
- 复杂 flow 是否用 Mermaid activity map 表达了 business mainline、actors、conditions、branches、joins、exceptions 和 cross-role handoffs？
