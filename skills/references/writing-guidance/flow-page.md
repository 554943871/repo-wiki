...
- 简短流程定位。
- 参与者和触发条件。
- Activity map 或步骤表，用来表达业务主链路、actors、conditions、branches、joins、exceptions 和 cross-role handoffs。
- Sequence diagram 或步骤表，用来解释关键场景中的参与方协作和调用顺序。
- State table 或 state diagram，用来解释核心对象、流程或页面背后的稳定状态变化。
- 关键分支和异常。
- 相关 pages。
- 相关 modules。
- 相关 models。
...
| 表达 | 适合回答 | 何时使用 | 不适合 |
| --- | --- | --- | --- |
| Activity map | 谁在什么条件下做什么，主链路如何分支、汇合、异常退出和跨角色交接 | flow 有多个 actor、业务条件、异常路径、人工介入或跨系统/跨角色交接时 | 普通 Controller -> Service -> Repository 调用链，或只能列出文件和 payload 的实现说明 |
...
| Prose | 范围、背景、证据限制、导航关系或很短的线性流程 | 信息简单，或需要保留不确定性、证据说明和读者下一步时 | 承载复杂分支、异常路径和跨角色交接的长段落 |
...
- 复杂 flow 是否用 activity map 或等价步骤表表达了 business mainline、actors、conditions、branches、joins、exceptions 和 cross-role handoffs？
